...
def calculate_clustering_metrics(y_true: np.ndarray, y_pred: np.ndarray, n_clusters: int) -> dict:
    ...
    true_to_idx = {l: i for i, l in enumerate(true_uniq)}
    ...
    idx_to_true = {i: l for l, i in true_to_idx.items()}
    ...
    try:
        metrics["Pairwise_Precision"], metrics["Pairwise_Recall"], metrics["Pairwise_F1"] = \
    ...
    if len(mapped_v) > 0:
        try:
            metrics.update({
                "Accuracy":  accuracy,
                "Precision": precision_score(y_true_v, mapped_v, average='macro', zero_division=0),
                "Recall":    recall_score(y_true_v, mapped_v, average='macro', zero_division=0),
                "Macro_F1":  f1_score(y_true_v, mapped_v, average='macro', zero_division=0),
                "Micro_F1":  f1_score(y_true_v, mapped_v, average='micro', zero_division=0),
            })
        except Exception as e:
