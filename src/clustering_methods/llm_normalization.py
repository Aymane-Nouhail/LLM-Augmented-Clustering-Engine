...
def _process_doc(
    ...
    try:
        ...
        normalized = response.strip() if response and response not in ("ERROR", "") else document
...
def cluster_via_llm_normalization(
    ...
    if os.path.exists(cache_file):
        try:
            ...
            print(f"  Loaded normalizations from cache: {cache_file} ({len(normalized_map)} docs)")
    ...
    if not normalized_map:
        ...
        print(f"  Querying LLM for canonical forms ({n_samples} docs, up to {max_workers} workers)...")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for future in tqdm(concurrent.futures.as_completed(futures), total=n_samples, desc="Normalizing"):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {
            executor.submit(
                _embed_normalized,
                i, documents[i], normalized_map.get(i, documents[i]),
    ...
    concat_features = np.hstack([np.array(original_features), np.array(expanded_features)])

    try:
        clusters = KMeans(n_clusters=n_clusters, random_state=0, n_init="auto").fit_predict(concat_features)
