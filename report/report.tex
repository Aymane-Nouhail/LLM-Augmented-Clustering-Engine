...
\twocolumn[{%
  ...
  \begin{abstract}
    ...
    predictor (precision\,0.72, specificity\,0.96 on Bank77 pairwise queries)
...
Text clustering is central to many NLP pipelines, from intent discovery in
...
K-Means, but raw embeddings capture surface similarity and can merge
semantically distinct intents that share vocabulary, or split paraphrases
that share the same intent.
...
Most must-links bridge nearby clusters in embedding space, while cannot-links
cross wide gaps between distinct intents, providing the semi-supervised
signal PCKMeans relies on.
A small number of long-range must-link edges reflect oracle errors
(precision 0.72; see \S\ref{sec:oracle}).
...
\caption{LLM-generated pairwise constraints on the Bank77 t-SNE.
  \textbf{Green} edges are must-link (YES) pairs;
  \textbf{red} edges are cannot-link (NO) pairs.
...
\textbf{Tweet}: 2\,472 tweets, 89 topic categories.
...
among multi-document LLM methods, and additionally removes the requirement
for a known $k$.
...
\caption{LLM oracle accuracy on Bank77 pairwise queries.
  ...
  The oracle achieves high precision on YES predictions (TP rate\,$\approx$\,0.72)
  and very high specificity on NO predictions (0.96), meaning almost all
  cannot-links correctly span different-class pairs.}
\label{fig:oracle}
\end{figure}

The oracle achieves precision\,$0.72$, recall\,$0.24$, and specificity\,$0.96$.
...
This asymmetric profile (conservative YES, reliable NO) is well-suited for
PCKMeans: a wrong cannot-link destroys cluster integrity, while a missed
must-link is recoverable.
...
The oracle's precision (0.72) is insufficient to suppress this noise at fine
granularities.
...
\textbf{LLM Normalization} achieves its largest gains on Bank77
(ACC\,0.649, best overall) and CLINC (NMI\,0.922, tied best), while on
Tweet it falls just below the K-Means baseline (0.640 vs.\ 0.649).
...
Figure~\ref{fig:paraphrase} shows that three LLM-generated paraphrases of
the same document cluster tightly around a shared semantic region.
\textbf{LLM Paraphrase Ensemble} achieves the best CLINC ACC (0.787) and
best Bank77 NMI (0.819), confirming that ensemble averaging reduces
surface-form variance.
...
exceed the true $k$.
The resulting over-split clusters are internally pure (hence the high precision
and NMI), but smaller than Hungarian matching expects, which depresses ACC.
...
alone.
Across both datasets the full pipeline costs under \$0.47 total,
showing that LLM-augmented clustering is affordable even on a student budget.
...
(precision\,$0.72$), which is the right profile for constraint-based
clustering methods.
...
classes.
The K-Means confusion heatmap highlights specific families of easily confused
intents and shows where LLM oracle effort is best directed.
